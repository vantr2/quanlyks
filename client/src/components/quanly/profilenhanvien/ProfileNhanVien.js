...
const ProfileNhanVien = () => {
  ...
  const [loiCapPhep, setLoiCapPhep] = useState("");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res.data.status === "ok") {
          if (res.data.data.nhanvien === undefined) {
            setLoiCapPhep("Tài khoản bạn sử dụng chưa được cấp phép.");
          } else {
  ...
  return (
    <div>
      {loiCapPhep !== "" ? (
        <p className="text-center text-danger">{loiCapPhep}</p>
      ) : (
